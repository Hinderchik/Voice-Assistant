...
API_TOKEN = '8219623838:AAE1hIIT1N-OWp0iYY4-DK9ZQFTFN78vHwk'
ADMIN_CHAT_ID = -1003111963427
...
def get_products_keyboard():
    keyboard = [
        [InlineKeyboardButton("💰 50 г → 75₽", callback_data="product_50")],
        [InlineKeyboardButton("💰 165 г → 190₽", callback_data="product_165")],
        [InlineKeyboardButton("💰 625 г → 590₽", callback_data="product_625")],
        [InlineKeyboardButton("💰 1625 г → 1450₽", callback_data="product_1625")],
        [InlineKeyboardButton("💰 6750 г → 5900₽", callback_data="product_6750")]
    ]
...
async def process_product_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    product_data = {
        'product_50': {'gold': '50 г', 'price': '75₽'},
        'product_165': {'gold': '165 г', 'price': '190₽'},
        'product_625': {'gold': '625 г', 'price': '590₽'},
        'product_1625': {'gold': '1625 г', 'price': '1450₽'},
        'product_6750': {'gold': '6750 г', 'price': '5900₽'}
    }
...
❓ *Возникли проблемы?* Свяжитесь с поддержкой: @Skuuuchn
    ...
    await query.message.reply_text(payment_text, reply_markup=get_check_status_keyboard(), parse_mode='Markdown')
    
    # Удаляем инлайн-клавиатуру из оригинального сообщения
...
# Обработчик команды отказа
async def reject_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик команды ?отказ {текст}"""
    # Проверяем формат команды
    if not context.args:
        await update.message.reply_text("❌ Используйте: ?отказ {причина отклонения}")
        return
    ...
    if not user_id or not message_id:
        await update.message.reply_text("❌ Нет активной заявки для отклонения!")
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif message_text.startswith('?отказ'):
...
def register_handlers():
    ...
    application.add_handler(MessageHandler(filters.TEXT & filters.Regex("^📋 Проверить статус покупки$"), check_purchase_status))
    application.add_handler(MessageHandler(filters.TEXT & filters.Regex("^⭐ Оставить отзыв$"), request_review))
    application.add_handler(MessageHandler(filters.TEXT & filters.Regex(r'^/start'), handle_message))
    application.add_handler(MessageHandler(filters.TEXT & filters.Regex(r'^\?отказ'), handle_message))

# Основная функция
async def main():
    """Запуск приложения"""
    print("🤖 Запуск Telegram бота в PyDroid 3...")
